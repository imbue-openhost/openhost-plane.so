Plane.so all-in-one container for Cloud in a Bottle. Bundles everything into a single Docker image:
...
- Cloud in a Bottle federated identity auth proxy

## TODO

- the Cloud in a Bottle auth stuff is still pretty janky. eg it'll spit out errors for ~2 mins on first boot until plane migrates itself or something
...
The Cloud in a Bottle router sets an `X-OpenHost-Is-Owner: true` header for authenticated zone owners. The auth proxy (`openhost_auth.py`) uses this to auto-create Django sessions for the owner.

Guest users authenticate via Cloud in a Bottle's federated identity flow:
...
- Redirects unauthenticated browser visitors to the Cloud in a Bottle identity login

## Deploying

Deploy via the Cloud in a Bottle router dashboard — point it at this repo. The app will be available at `{app_name}.{zone_domain}` via subdomain routing.
...
- `openhost_auth.py` — Cloud in a Bottle federated auth proxy (auto-setup, owner auto-login, guest identity login)
